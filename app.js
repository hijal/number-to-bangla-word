//  DOM
var enteredValue = document.getElementById("inputValue");
var invalidInput = document.getElementById("inputInvalid");
var result = document.getElementById("result-show");
var convertedWord = document.getElementById("converted-word");
var submittedNumber = document.getElementById("submit-number");
...
var ERROR_MESSAGE = {
  INVALID: "একটি বৈধ নম্বর লিখুন",
  IS_FINITE: "একটি সীমাবদ্ধ সংখ্যা নয়",
  IS_SAFE: "সংখ্যা একটি নিরাপদ সংখ্যা নয়, এটি হয় খুব বড় বা খুব ছোট",
};
...
var convertToWord = function (number) {
  ...
  if (!isFinite(number)) {
    invalidInput.textContent = ERROR_MESSAGE.IS_FINITE;
    return;
  }
  if (!isSafeNumber(number)) {
    invalidInput.textContent = ERROR_MESSAGE.IS_SAFE;
    return;
  }
...
// initially result card display none
result.style.display = "none";

submittedNumber.addEventListener("click", function () {
  let val = enteredValue.value;

  if (isNaN(val)) {
    invalidInput.textContent = ERROR_MESSAGE.INVALID;
    result.style.display = "none";
  } else {
    invalidInput.textContent = "";
    var convertedNumber = convertToWord(+val); // +val is used to convert string to number
    result.style.display = "block";

    convertedWord.textContent = convertedNumber;
  }
});
